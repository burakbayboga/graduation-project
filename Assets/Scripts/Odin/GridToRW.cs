...
public class GridToRW : MonoBehaviour {
	...
	static Vector2 GetVacantPoint(int xStartRW, int yStartRW, AStarMap mapHook){
		...
		Collider[] units = Physics.OverlapBox(new Vector3(xStartRW + 2.5f, yStartRW + 2.5f, 0f), new Vector3(2.5f, 0f, 0f), Quaternion.identity, layerMask);
		...
		for(int i=xStartRW; i < xStartRW+5; i++){
			for(int j=yStartRW; j < yStartRW+5; j++){
				if(map[i,j].walkable){
